...
namespace Dungeon
{
    public class DungeonTask
    {
        ...
        {
            var directPath = FindDirectPath(map);

            if (directPath != null && directPath.Any(map.Chests.Contains))
            {
           ...
           IEnumerable<SinglyLinkedList<Point>> pathToFinish, Map map)
        {
            ...
            foreach (var start in pathFromStart)
            {
                ...
                foreach (var finish in pathToFinish)
                {
                    ...
                    {
                        var temp = CombinePaths(start, finish);
        ...
        private static Point[] CombinePaths(SinglyLinkedList<Point> start, SinglyLinkedList<Point> finish)
        {
        ...
        private static Point[] FindDirectPath(Map map)
        {
            return BfsTask
